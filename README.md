...
- `bot.js` — логіка ботів (mineflayer), консольний режим + API для GUI.
- `main.js` — головний процес Electron (створює вікно, IPC).
- `preload.js` — міст між Renderer і main-процесом (безпечно віддає `mcBots` API).
- `renderer.js` — логіка форми в GUI.
- `index.html` — інтерфейс програми.
- `package.json` — залежності, скрипти запуску.
- `.gitignore` — виключає `node_modules`, логи та тимчасові файли.
